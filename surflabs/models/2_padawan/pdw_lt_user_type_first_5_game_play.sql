...
(   
    select 
	*,
	rank() over(partition by user_id, game_type order by created_at) as game_number
    from
    {{ ref('stg_padme_game_play') }}
	...
	( select cug from
	( 
	select concat(user_id, game_type) cug, max(game_number) mgn from {{ this }}
	{{ dbt_utils.group_by(1) }}
	) x where mgn >= 5 ) a 
	on new_users_game_type.cug = a.cug and a.cug is null
        )
    {% endif %}
)
