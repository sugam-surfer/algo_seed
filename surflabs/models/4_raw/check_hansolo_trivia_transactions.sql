{{
    config(
    	...
    	post_hook	= ["truncate table ahsokatano_4_raw.old_hansolo_trivia_transactions"],
        tags 		= ['on_chain', 'trivia', 'reseed', 'check', 'warehouse'],
    )
}}
...
source_data_2 as (
    select
        * , null as user_id
    from {{ source('hansolo', 'trivia_transactions') }}
        {% if is_incremental() -%}
...
old_data as (
    select
        * , 
        {{ dbt_date.convert_timezone(current_timestamp(), source_tz="UTC") }} as inserted_at,
        'NA' surr_key 
    from {{ ref('old_hansolo_trivia_transactions') }}	
),
...
pre_exist_label as (
    SELECT 
    	sic.*,
    	CASE
        	WHEN EXISTS (SELECT 1 FROM ahsokatano_4_raw.error_hansolo_trivia_transactions d1 WHERE sic.id = d1.id) THEN 'error'
        	WHEN EXISTS (SELECT 1 FROM ahsokatano_4_raw.raw_hansolo_trivia_transactions d2 WHERE sic.id = d2.id) THEN 'dest'
        	WHEN EXISTS (SELECT 1 FROM ahsokatano_4_raw.qa_hansolo_trivia_transactions d3 WHERE sic.id = d3.id) THEN 'qa'
        	ELSE NULL
...
pre_exist_final_error as (
    ...
    left join ahsokatano_4_raw.error_hansolo_trivia_transactions n
    on m.id = n.id
...
pre_exist_final_qa as (
    select 
      	...
      	m.dest_table_type
    from pre_exist_label m
    left join ahsokatano_4_raw.qa_hansolo_trivia_transactions n
    on m.id = n.id
...
error_handling as (
    SELECT 
      	main.*,
      	CASE
        	when main.ident_value not in (select ident_value from {{ ref('raw_user_ident') }}) then 11
          	WHEN (
            		SELECT COUNT(*) 
            		FROM non_pre_exist 	
				WHERE id = main.id
        	...
        	WHEN EXISTS (
            		SELECT 1 
            		FROM ahsokatano_4_raw.raw_hansolo_trivia_transactions 
              			WHERE id = main.id 
...
non_pre_exist_final as (
    select 
      	* exclude (issue_number), 
      	case issue_number 
		when 0 then 'OK'
        	when 11 then 'ident_value NA in raw_user_ident'
