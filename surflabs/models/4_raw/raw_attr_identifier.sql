...
select * from {{ ref('input_gr_attr_identifier') }}
    union all
select * from {{ ref('input_la_attr_identifier') }}
    union all
select * from ahsokatano_4_raw.input_tz_attr_identifier

/*    
with

input_attr_identifier as (
select * from ahsokatano_4_raw.input_gr_attr_identifier
    union all
select * from ahsokatano_4_raw.input_la_attr_identifier
    union all
select * from ahsokatano_4_raw.input_tz_attr_identifier
...
input_hashed AS (
    SELECT
        *,
        HASH(identifier) AS pk_hash, 
        HASH(*) AS row_hash 
    FROM
        input_attr_identifier
),
...
from
    ...
    SELECT
        *,
        HASH(identifier) AS pk_hash 
    FROM ahsokatano_4_raw.raw_attr_identifier 
    ) a
    left join
    (
    select 
