{{
	...
	post_hook	= ["truncate table ahsokatano_4_raw.old_user_app"],
...
SELECT 
    sic.*,
    CASE
        WHEN EXISTS (SELECT 1 FROM ahsokatano_4_raw.error_user_app d1 WHERE sic.user_id = d1.user_id and sic.app_id = d1.app_id) THEN 'error'
        WHEN EXISTS (SELECT 1 FROM ahsokatano_4_raw.raw_user_app d2 WHERE sic.user_id = d2.user_id and sic.app_id = d2.app_id) THEN 'dest'
        WHEN EXISTS (SELECT 1 FROM ahsokatano_4_raw.qa_user_app d3 WHERE sic.user_id = d3.user_id and sic.app_id = d3.app_id) THEN 'qa'
        ELSE NULL
...
pre_exist_final_error as (
    ...
    left join ahsokatano_4_raw.error_user_app n
...
pre_exist_final_qa as (
	...
	m.dest_table_type
    from pre_exist_label m
    left join ahsokatano_4_raw.qa_user_app n
    on m.user_id = n.user_id
    and m.app_id = n.app_id
...
error_handling as (
	...
	CASE
		...
		when x.id is null then 12
       		...
       		WHEN EXISTS (
       			SELECT 1 
       			FROM ahsokatano_4_raw.raw_user_app 
       				WHERE user_id = main.user_id 
