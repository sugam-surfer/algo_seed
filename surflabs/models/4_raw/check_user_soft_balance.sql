...
{{
	...
	post_hook	= ["truncate table ahsokatano_4_raw.old_user_soft_balance"],
...
old_data as (
	...
	{{ dbt_date.convert_timezone(current_timestamp(), source_tz="UTC") }} as inserted_at,
        'NA' surr_key 
    from {{ ref('old_user_hard_balance') }}	
),
...
pre_exist_label as (
    SELECT 
    	sic.*,
    	CASE
        	WHEN EXISTS (SELECT 1 FROM ahsokatano_4_raw.error_user_soft_balance d1 WHERE sic.user_id = d1.user_id and sic.app_id = d1.app_id and sic.currency = d1.currency) THEN 'error'
        	WHEN EXISTS (SELECT 1 FROM ahsokatano_4_raw.raw_user_soft_balance d2 WHERE sic.user_id = d2.user_id and sic.app_id = d2.app_id and sic.currency = d2.currency) THEN 'dest'
        	WHEN EXISTS (SELECT 1 FROM ahsokatano_4_raw.qa_user_soft_balance d3 WHERE sic.user_id = d3.user_id and sic.app_id = d3.app_id and sic.currency = d3.currency) THEN 'qa'
        	ELSE NULL
...
pre_exist_final_error as (
    ...
    left join ahsokatano_4_raw.error_user_soft_balance n
...
pre_exist_final_qa as (
	...
	m.dest_table_type
    from pre_exist_label m
    left join ahsokatano_4_raw.qa_user_soft_balance n
    on m.user_id = n.user_id
    and m.app_id = n.app_id
    and m.currency = n.currency
...
error_handling as (
    SELECT 
		...
		when x.id is null then 12
        	...
        	WHEN EXISTS (
            		SELECT 1 
            		FROM ahsokatano_4_raw.raw_user_soft_balance 
            			WHERE user_id = main.user_id 
