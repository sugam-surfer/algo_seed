{{
	...
	post_hook	= ["truncate table ahsokatano_4_raw.old_USER_BALANCE_LOG"],
...
source_data_2 as (
    select
        *  
    from {{ source('padme','user_balance_log') }} 
        {% if is_incremental() -%}
...
pre_exist_label as (
	...
	sic.*,
	CASE
        	WHEN EXISTS (SELECT 1 FROM ahsokatano_4_raw.error_USER_BALANCE_LOG d1 WHERE sic.id = d1.id) THEN 'error'
        	WHEN EXISTS (SELECT 1 FROM ahsokatano_4_raw.raw_USER_BALANCE_LOG d2 WHERE sic.id = d2.id) THEN 'dest'
        	WHEN EXISTS (SELECT 1 FROM ahsokatano_4_raw.qa_USER_BALANCE_LOG d3 WHERE sic.id = d3.id) THEN 'qa'
        	ELSE NULL
...
pre_exist_final_error as (
    ...
    left join ahsokatano_4_raw.error_user_balance_log n
    on m.id = n.id
...
pre_exist_final_qa as (
	...
	m.dest_table_type
    from pre_exist_label m
    left join ahsokatano_4_raw.qa_user_balance_log n
    on m.id = n.id
